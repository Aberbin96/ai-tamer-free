...
class LicenseVerifier
{
	 ...
	 */
	public static function issue_token(string $agent_name, int $days = 365, string $scope = 'global'): string
	{
	...
	public static function get_tokens(array $args = array()): array
	{
		...
		if (! empty($args['s'])) {
			...
			$tokens = array_filter(
				$tokens,
				function ($t) use ($s) {
					return strpos(strtolower((string) ($t['agent'] ?? '')), $s) !== false || strpos(strtolower((string) ($t['token'] ?? '')), $s) !== false;
				}
			);
		}
