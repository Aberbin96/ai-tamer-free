...
if (
	...
	$issued_token = LicenseVerifier::issue_token( $agent_name, $days );
}

$tokens = LicenseVerifier::get_tokens();
?>
<div class="wrap aitamer-wrap">
	...
	<?php if ( isset( $_GET['aitamer_revoked'] ) ) : ?>
	...
	<?php if ( $issued_token ) : ?>
		<div class="notice notice-success">
			...
			<code class="aitamer-token-block"><?php echo esc_html( $issued_token ); ?></code>
			...
			<div class="aitamer-table-responsive">
				...
				<tbody>
					<?php foreach ( $tokens as $i => $t ) :
						$is_expired = (int) $t['exp'] < time();
					?>
					<tr>
						<td><strong><?php echo esc_html( $t['agent'] ); ?></strong></td>
						<td class="mono"><?php echo esc_html( wp_date( 'Y-m-d', $t['issued_at'] ) ); ?></td>
						<td class="mono"><?php echo esc_html( wp_date( 'Y-m-d', $t['exp'] ) ); ?></td>
						<td>
							<span class="aitamer-badge-status <?php echo $is_expired ? 'expired' : 'active'; ?>">
								<?php echo $is_expired ? esc_html__( 'Expired', 'ai-tamer' ) : esc_html__( 'Active', 'ai-tamer' ); ?>
							</span>
						</td>
						<td><code><?php echo esc_html( substr( $t['token'], 0, 28 ) . '…' ); ?></code></td>
						<td>
							<form method="post" style="display:inline;" onsubmit="return confirm('<?php esc_attr_e( 'Revoke this token?', 'ai-tamer' ); ?>');">
								...
								<input type="hidden" name="revoke_index" value="<?php echo esc_attr( $i ); ?>">
